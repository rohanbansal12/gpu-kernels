...
from gpu_kernels.ops.silu import silu_jax_jit, silu_torch, silu_torch_compile, silu_triton
from gpu_kernels.ops.silu.triton_impl import DEFAULT_BLOCK
...
def main() -> None:
    ...
    # Count SiLU as exp + add + div + multiply. Exact transcendental cost is
    # hardware-specific, but this keeps the suite comparable across variants.
    flops = 4 * args.n
    workload = Workload(
        op="silu",
        flops=flops,
    ...
    variants = {
        "torch": Variant(name="torch", fn=silu_torch, framework="torch"),
        "torch_compile": Variant(
            name="torch_compile",
            fn=silu_torch_compile,
            framework="torch",
        ),
    }
    ...
    if args.sweep_block is not None:
        if not runtime.has_triton() or not runtime.torch_cuda_available():
            parser.error("--sweep-block for silu requires Triton and CUDA")
