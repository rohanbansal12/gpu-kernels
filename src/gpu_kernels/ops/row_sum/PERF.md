...
- Not yet run on `H100!`; command: `uv run --extra cloud modal run modal_app.py::correctness --op row_sum`

## Results

Use `uv run python -m benchmarks.history_report row_sum --latest` after benchmarking, then
...
- Notes: row_sum is a low-arithmetic-intensity reduction and should mainly track memory bandwidth.
