...
- >=70% speed-of-light @ 256 Mi elements, bf16, tanh approximation, H100!

## Correctness

- Not yet run on `H100!`; command: `uv run --extra cloud modal run modal_app.py::correctness --op gelu`

## Results

Use `uv run python -m benchmarks.history_report gelu --latest` after benchmarking, then
...
- Notes: GELU includes transcendental/approximation work, so SoL is clearer than raw BW.
