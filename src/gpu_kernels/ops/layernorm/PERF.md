...
- Not yet run on `H100!`; command: `uv run --extra cloud modal run modal_app.py::correctness --op layernorm`

## Results

Use `uv run python -m benchmarks.history_report layernorm --latest` after benchmarking, then
...
- Notes: LayerNorm mixes reductions, elementwise math, and multiple memory streams.
